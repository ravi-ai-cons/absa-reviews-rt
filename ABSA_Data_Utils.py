...
class ABSATokenizer(BertTokenizer):
    def subword_tokenize(self, tokens):  # for AE
        split_tokens = []
        idx_map = []
...
class AscProcessor(DataProcessor):
    ...
    def get_test_examples(self, data, fn="test.json"):
        """See base class."""
...
def convert_examples_to_features(examples, max_seq_length, tokenizer, mode):
...
#         label_map[label] = i
    ...
    for (ex_index, example) in enumerate(examples):
        if mode != "ae":
            ...
            tokens_a, example.idx_map = tokenizer.subword_tokenize([token.lower() for token in example.text_a])
